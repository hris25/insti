...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-900">Modifier le club</h1>
        ...
        <div class="bg-white shadow-md rounded-lg p-6">
            <form action="{{ route('admin.clubs.update', $club) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="nom" id="nom" value="{{ old('nom', $club->nom) }}" required
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 @error('description') border-red-500 @enderror">{{ old('description', $club->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    @if($club->image)
                        <div class="mt-2 mb-4">
                            <img src="{{ asset('storage/' . $club->image) }}" alt="{{ $club->nom }}" class="h-32 w-32 object-cover rounded-lg">
                        </div>
                    @endif
                    ...
                    <p class="mt-1 text-sm text-gray-500">Format recommandé : JPG, PNG. Taille max : 2MB. Laissez vide pour conserver l'image actuelle.</p>
                    @error('image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="horaires" id="horaires" value="{{ old('horaires', $club->horaires) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="lieu" id="lieu" value="{{ old('lieu', $club->lieu) }}" required
                    ...
                    <div id="activites-container" class="space-y-2">
                        @foreach($club->activites as $activite)
                            <div class="flex space-x-2">
                                <input type="text" name="activites[]" value="{{ $activite }}" required
